...
export default function AdminPage() {
  ...
  const generateOrgCode = () =>
    'SRN-' + Math.random().toString(36).substr(2, 6).toUpperCase();
  ...
  const handleRejectOrg = async () => {
    ...
    await sendEmail('org_rejected', selectedItem.email, {
      orgName: selectedItem.orgName, reason: rejectReason,
    });
  ...
  const handleRejectResponder = async () => {
    ...
    if (selectedItem.email) {
      await sendEmail('responder_rejected', selectedItem.email, {
        responderName: selectedItem.name, reason: rejectReason,
      });
    }
    setShowRejectModal(false);
    setRejectReason('');
  ...
  const pendingOrgs = organisations.filter(
      ...
      <main className="min-h-screen bg-[#050505] text-white flex items-center justify-center px-6">
        <div className="w-full max-w-sm">
          ...
          <p className="text-white/30 text-sm text-center mb-8">
            This area is restricted to Siren administrators only
          </p>
          {error && (
    ...
    <div className="min-h-screen bg-[#050505] text-white flex">
      ...
      <div className="w-64 bg-[#0a0a0a] border-r border-white/[0.06] flex flex-col p-6 fixed h-full">
        ...
        <div className="border-t border-white/[0.06] pt-4">
          <button
            onClick={() => {
              sessionStorage.removeItem('sirenAdminAuth');
      ...
      <div className="ml-64 flex-1 p-8">
          ...
          <div>
            ...
            {pendingOrgs.length > 0 && (
              <div className="bg-white/[0.02] border border-[#cc0000]/30 rounded-2xl p-6 mb-6">
                ...
                {pendingOrgs.slice(0, 3).map((org) => (
                  <div
                    key={org.id}
                    className="flex items-center justify-between bg-white/[0.02] rounded-xl p-4 mb-3 last:mb-0"
                  >
                    <div>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-6">
                ...
                {users.length === 0 && (
                  <p className="text-white/20 text-sm text-center py-8">No citizens yet</p>
                )}
              </div>
            </div>
          </div>
        ...
        {activeTab === 'organisations' && (
          <div>
            ...
            <p className="text-white/30 text-sm mb-8">
              Review and verify emergency response organisations
            </p>
              ...
              <div className="mb-8">
                ...
                <div className="flex flex-col gap-4">
                  {pendingOrgs.map((org) => (
                    <div
                      key={org.id}
                      className="bg-white/[0.02] border border-[#cc0000]/20 rounded-2xl p-6"
                    >
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          <h3 className="text-white font-bold text-lg">{org.orgName}</h3>
                          <p className="text-white/40 text-sm">
                            {org.orgType} • {org.state}, {org.lga}
                          </p>
                        </div>
                ...
                <thead>
                  <tr className="border-b border-white/[0.06]">
                    {['Organisation', 'Type', 'State', 'Org Code', 'Status'].map((h) => (
                      <th key={h} className="text-left text-white/30 text-xs font-semibold px-6 py-4 uppercase tracking-widest">
                        {h}
                      </th>
                    ))}
                  </tr>
        ...
        {activeTab === 'responders' && (
          <div>
                ...
                <div className="flex flex-col gap-4">
                  {pendingResponders.map((r) => (
                    ...
                    >
                      ...
                      <div className="flex gap-3">
                        <button
                          onClick={() => handleApproveResponder(r)}
                          className="flex-1 bg-[#1a3a1a] border border-[#00cc44] text-[#00cc44] py-3 rounded-xl text-sm font-semibold hover:bg-[#00cc44] hover:text-white transition"
                        >
                          ✅ Verify
                        </button>
            ...
            <h2 className="text-white font-bold mb-4">
              Verified ({verifiedResponders.length})
            </h2>
            <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl overflow-hidden">
              <table className="w-full">
                <thead>
                  ...
                  {emergencies.length === 0 ? (
                    ...
                    emergencies.map((em) => (
                      <tr key={em.id} className="border-b border-white/[0.04] last:border-0 hover:bg-white/[0.02]">
                        ...
                        <td className="px-6 py-4 text-white/40 text-sm max-w-xs truncate">
                          {em.description}
                        </td>
              ...
              {[
                { title: 'Total Emergencies', value: emergencies.length, sub: 'All time' },
                {
                  ...
                  value: emergencies.length > 0
                    ? `${Math.round((resolvedEmergencies.length / emergencies.length) * 100)}%`
                    : '0%',
                  sub: 'Resolved vs total',
                },
                {
                  title: 'Active Responders',
                  value: verifiedResponders.filter((r: any) => r.isAvailable).length,
                  sub: 'Online now',
                },
